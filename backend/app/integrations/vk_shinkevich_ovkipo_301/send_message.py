...
class VkSendMessageIntegration(BaseIntegration):
    ...
    def metadata(self) -> IntegrationMetadata:
        return IntegrationMetadata(
            id="vk_send_message_shinkevich_ovkipo_301",
            version="1.0.0",
            name="VK Send Message (Shinkevich)",
            description="Отправка сообщения (Shinkevich)",
            category="messaging",
            ...
            config_schema={
                ...
                "properties": {
                    ...
                    "message": {"type": "string", "title": "Текст"}
                },
    ...
    async def execute(self, config, credentials_resolver, bot_id, logger) -> Dict[str, Any]:
        ...
        try:
            def _send():
                return vk_api.VkApi(token=token).get_api().messages.send(
