...
class TelegramSendLocationIntegration(BaseIntegration):
    ...
    def metadata(self) -> IntegrationMetadata:
        return IntegrationMetadata(
            ...
            config_schema={
                ...
                "properties": {
                    ...
                    "latitude": {
                        "type": "number",
                        "title": "Latitude",
                        "description": "Широта (например: 55.7558)"
                    },
                    "longitude": {
                        "type": "number",
                        "title": "Longitude", 
                        "description": "Долгота (например: 37.6173)"
                    }
                }
            },
            ...
            examples=[
                {
                    "title": "Отправка координат Кремля",
                    "config": {
                ...
                {
                    "title": "Текущая геопозиция пользователя",
                    "config": {
                        "chat_id": "{$user.telegram_chat_id$}",
                        "latitude": 55.7558,
                        "longitude": 37.6173
    ...
    async def execute(
        ...
        if not chat_id or latitude is None or longitude is None:
            await logger.error("chat_id, latitude and longitude are required")
            return {
                "response": {
                    "ok": False,
                    "error_code": 400,
        ...
        try:
            ...
            if not (-90 <= latitude <= 90) or not (-180 <= longitude <= 180):
                await logger.error(f"Invalid coordinates: latitude={latitude}, longitude={longitude}")
                return {
                    "response": {
                        "ok": False,
                        "error_code": 400,
                        "description": "Invalid coordinates. Latitude must be between -90 and 90, longitude between -180 and 180"
                    }
                }
        except (ValueError, TypeError):
            await logger.error(f"Invalid coordinate format: latitude={latitude}, longitude={longitude}")
            return {
                "response": {
                    "ok": False,
                    "error_code": 400,
                    "description": "Invalid coordinate format. Latitude and longitude must be numbers"
                }
            }
