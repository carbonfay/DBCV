...
class TelegramGetChatMembersCountIntegration(BaseIntegration):
    """Интеграция для получения количества участников чата в Telegram через python-telegram-bot."""
    ...
    def metadata(self) -> IntegrationMetadata:
        return IntegrationMetadata(
            ...
            description="Получение количества участников в Telegram чате через Bot API",
            category="messaging",
            ...
            config_schema={
                ...
                "properties": {
                    "chat_id": {
                        "type": "string",
                        "title": "Chat ID",
                        "description": "ID чата или канала (можно использовать переменные: {$user.telegram_chat_id$})"
                    }
                }
            },
            ...
            examples=[
                {
                    "title": "Получение количества участников",
                    "config": {
    ...
    async def execute(
        ...
        try:
            ...
            count = await bot.get_chat_member_count(chat_id=str(chat_id))
