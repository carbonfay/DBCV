"""Bitrix24: Get Contact integration (crm.contact.get).

Получает контакт CRM Bitrix24 по его ID через webhook или domain+access_token используя httpx.
"""
...
class Bitrix24GetContactIntegration(BaseIntegration):
    """Интеграция Bitrix24: получение контакта (crm.contact.get)."""

    @property
    def metadata(self) -> IntegrationMetadata:
        return IntegrationMetadata(
            id="bitrix24_get_contact",
            version="1.0.0",
            name="Bitrix24 Get Contact",
            description="Получение контакта из Bitrix24 по ID (crm.contact.get)",
            category="crm",
            ...
            config_schema={
                ...
                "required": ["id"],
                "properties": {
                    "id": {"type": "integer", "title": "ID", "description": "ID контакта в Bitrix24"}
                },
            ...
            examples=[
                {"title": "Get contact by ID", "config": {"id": 123}},
            ],
        )
    ...
    async def execute(
        ...
        """Выполняет запрос crm.contact.get по ID.
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                # webhook: POST to webhook_url/crm.contact.get
                if webhook_url:
                    try:
                        url = webhook_url.rstrip("/") + "/crm.contact.get"
                        resp = await client.post(url, json=request_params)
                ...
                # domain + token: GET to /rest/crm.contact.get.json?id=...&auth=...
                if domain and access_token:
                    try:
                        ...
                        url = f"{base}/rest/crm.contact.get.json"
                        params_q = {"id": contact_id, "auth": access_token}
