...
from app.integrations.medicine.get_doctor_info_npi_registry import MedicineGetDoctorInfoIntegration
...
class _MockResponse:
    ...
    def __init__(self, status_code: int = 200, json_data: dict | None = None, text: str = ""):
        ...
        self._json_data = json_data or {"results": [{"basic": {"first_name": "John"}, "number": "1234567890"}]}
        self.text = text
...
async def test_execute_success(monkeypatch):
    ...
    integration = MedicineGetDoctorInfoIntegration()
    ...
    assert result["response"]["result"]["results"][0]["number"] == "1234567890"
...
async def test_execute_real_npi_registry():
    ...
    # Используем известный валидный NPI (может потребоваться обновление, если данные изменятся)
    npi = "1234567890"
    result = await integration.execute(
        config={"npi": npi},
        credentials_resolver=credentials_resolver,
