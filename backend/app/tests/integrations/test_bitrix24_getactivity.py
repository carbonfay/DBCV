"""Tests for Bitrix24 Get Activity integration."""
import pytest
from uuid import UUID
from unittest.mock import AsyncMock, MagicMock, patch

from app.integrations.bitrix24.get_activity import Bitrix24GetActivityIntegration
...
@pytest.fixture
def integration():
    return Bitrix24GetActivityIntegration()
...
def test_metadata(integration):
    ...
    assert md.id == "bitrix24_get_activity"
    assert md.version == "1.0.0"
    assert md.name == "Bitrix24 Get Activity"
...
async def test_execute_webhook_success(integration, logger, bot_id):
    ...
    mock_response.json = MagicMock(return_value={"result": {"ID": 777, "SUBJECT": "Call"}})

    mock_client = AsyncMock()
    mock_client.__aenter__.return_value = mock_client
    mock_client.post = AsyncMock(return_value=mock_response)

    with patch("app.integrations.bitrix24.get_activity.httpx.AsyncClient", return_value=mock_client):
    ...
    assert result["response"]["result"]["result"]["ID"] == 777
...
async def test_execute_domain_token_success(integration, logger, bot_id):
    ...
    mock_response = MagicMock()
    mock_response.raise_for_status = MagicMock()
    mock_response.json = MagicMock(return_value={"result": {"ID": 888, "SUBJECT": "Email"}})
    ...
    assert result["response"]["ok"] is True
    assert result["response"]["result"]["result"]["ID"] == 888
