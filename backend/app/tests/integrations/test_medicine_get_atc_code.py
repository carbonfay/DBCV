...
from app.integrations.medicine.get_atc_code_rxnav import MedicineGetATCCodeIntegration
...
class _MockResponse:
    ...
    def __init__(self, status_code: int = 200, json_data: dict | None = None, text: str = ""):
        ...
        self._json_data = json_data or {"propConceptGroup": {"propConcept": [{"propValue": "A01AB"}]}}
        self.text = text
...
async def test_execute_success(monkeypatch):
    ...
    integration = MedicineGetATCCodeIntegration()
    ...
    assert result["response"]["result"]["propConceptGroup"]["propConcept"][0]["propValue"] == "A01AB"
    credentials_resolver.get_default_for.assert_not_called()


@pytest.mark.asyncio
async def test_execute_missing_rxcui(monkeypatch):
    """Integration validates required rxcui."""
...
async def test_execute_real_rxnav():
    """Real call to RxNav ATC property API without credentials."""
