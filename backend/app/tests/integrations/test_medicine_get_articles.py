...
from app.integrations.medicine.get_articles_pubmed import MedicineGetArticlesIntegration
...
class _MockResponse:
    ...
    def __init__(self, status_code: int = 200, json_data: dict | None = None, text: str = ""):
        ...
        self._json_data = json_data or {"esearchresult": {"idlist": ["1"]}}
        self.text = text
...
class _MockAsyncClient:
    ...
    def __init__(self, search_response: _MockResponse | None = None, summary_response: _MockResponse | None = None, *_, **__):
        self._search_response = search_response or _MockResponse()
        self._summary_response = summary_response or _MockResponse(json_data={"result": {"uids": ["1"], "1": {"title": "Test article"}}})
        self.get = AsyncMock(side_effect=[self._search_response, self._summary_response])
...
@pytest.mark.asyncio
async def test_execute_real_pubmed():
    """Real call to PubMed API without credentials."""
    ...
    log_file = logs_dir / f"{Path(__file__).stem}_{ts}_test_execute_real_pubmed.json"
