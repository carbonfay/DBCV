"""Tests for Bitrix24 Create Deal integration."""
import pytest
from uuid import UUID
from unittest.mock import AsyncMock, MagicMock, patch

from app.integrations.bitrix24.create_deal import Bitrix24CreateDealIntegration
...
@pytest.fixture
def integration():
    return Bitrix24CreateDealIntegration()
...
def test_metadata(integration):
    ...
    assert meta.id == "bitrix24_create_deal"
    assert meta.version == "1.0.0"
    assert meta.name == "Bitrix24 Create Deal"
...
async def test_execute_webhook_success(integration, credentials_resolver, logger, bot_id):
    ...
    mock_resp.json = MagicMock(return_value={"result": {"ID": 901}})
    mock_resp.text = "{\"result\": {\"ID\": 901}}"
    mock_resp.raise_for_status = MagicMock()

    client_mock = MagicMock()
    client_mock.post = AsyncMock(return_value=mock_resp)

    with patch("app.integrations.bitrix24.create_deal.httpx.AsyncClient") as AsyncClientMock:
        AsyncClientMock.return_value.__aenter__.return_value = client_mock

        result = await integration.execute(
            config={"fields": {"TITLE": "Deal 1", "OPPORTUNITY": 100}},
    ...
    assert result["response"]["result"]["result"]["ID"] == 901


@pytest.mark.asyncio
async def test_execute_domain_token_success(integration, credentials_resolver, logger, bot_id):
    credentials_resolver.get_default_for = AsyncMock(return_value={
        "payload": {"domain": "example.bitrix24.ru", "access_token": "tokxyz"}
    })

    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.json = MagicMock(return_value={"result": {"ID": 902}})
    mock_resp.text = "{\"result\": {\"ID\": 902}}"
    mock_resp.raise_for_status = MagicMock()
    ...
    with patch("app.integrations.bitrix24.create_deal.httpx.AsyncClient") as AsyncClientMock:
        ...
        result = await integration.execute(
            config={"fields": {"TITLE": "Deal 2", "CURRENCY_ID": "USD"}},
            credentials_resolver=credentials_resolver,
            bot_id=bot_id,
            logger=logger,
        )
    ...
    assert result["response"]["ok"] is True
    assert result["response"]["result"]["result"]["ID"] == 902
...
async def test_execute_api_error(integration, credentials_resolver, logger, bot_id):
    ...
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.json = MagicMock(return_value={"error": "ERR", "error_description": "Bad deal"})
    mock_resp.text = "{\"error\": \"ERR\", \"error_description\": \"Bad deal\"}"
    mock_resp.raise_for_status = MagicMock()
    ...
    assert result["response"]["ok"] is False
    assert result["response"]["error_code"] == 400
    assert "Bad deal" in result["response"]["description"]
