    ...
    <div class="modal" @click.stop>
      ...
      <div class="modal-body">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          >
            <BaseInput
              v-if="isFieldType(field.type, 'string') && !field.enum"
            ...
            />
              ...
              <BaseSelect
                v-else-if="isFieldType(field.type, 'string') && field.enum"
                v-model="formData[field.key]"
            ...
            <BaseInput
              v-else-if="isFieldType(field.type, 'number') || isFieldType(field.type, 'integer')"
              v-model="formData[field.key]"
              type="text"
              :label="field.title + (field.required ? ' *' : '')"
              :placeholder="field.description || field.title"
              labelColor="#FFFFFF"
              :error="errors[field.key]"
              @blur="coerceNumericField(field.key)"
            />
            
            <BaseCheckbox
              v-else-if="isFieldType(field.type, 'boolean')"
              v-model="formData[field.key]"
              :label="field.title + (field.required ? ' *' : '')"
            />
            
            <!-- Array поле - JSON редактор -->
            <div v-else-if="isFieldType(field.type, 'array')" class="json-field">
            ...
            <div v-else-if="isFieldType(field.type, 'object')" class="json-field">
              <label class="field-label">
                {{ field.title + (field.required ? ' *' : '') }}
              </label>
              <textarea
...
// Helper функция для проверки типов поля (поддерживает string и array типов)
    ...
    if (integration?.config_schema?.properties) {
      ...
      Object.entries(schema.properties).forEach(([key, prop]: [string, any]) => {
        if (prop.default !== undefined) {
          ...
          if (isFieldType(propType, 'object') || isFieldType(propType, 'array')) {
            initialJson[key] = JSON.stringify(prop.default, null, 2);
          }
        } else if (isFieldType(prop.type, 'object')) {
          initialData[key] = {};
          initialJson[key] = '{}';
        } else if (isFieldType(prop.type, 'array')) {
          initialData[key] = [];
          initialJson[key] = '[]';
        }
      });
...
// Функция для приведения числовых полей (поддержка запятой вместо точки)
const coerceNumericField = (key: string) => {
  ...
  if (typeof value === 'string') {
    ...
    try {
      ...
      // Оставляем как есть, если не можно конвертировать
    }
  }
};
...
const handleJsonInput = (key: string) => {
  ...
  if (!jsonString.trim()) {
    ...
    if (isFieldType(field?.type, 'object')) {
      formData.value[key] = {};
    } else if (isFieldType(field?.type, 'array')) {
      formData.value[key] = [];
    }
    return;
  }
  ...
  try {
    ...
    // Проверяем тип
    const field = formFields.value.find(f => f.key === key);
    if (isFieldType(field?.type, 'array') && !Array.isArray(parsed)) {
      errors.value[key] = 'Должен быть массив';
      return;
    }
    if (isFieldType(field?.type, 'object') && (Array.isArray(parsed) || typeof parsed !== 'object')) {
...
const validateForm = () => {
  ...
  props.integration.config_schema.required.forEach((key: string) => {
    ...
    if (isFieldType(prop?.type, 'object') || isFieldType(prop?.type, 'array')) {
      const jsonString = formDataJson.value[key] || '';
      if (!jsonString.trim()) {
      ...
      try {
        const parsed = JSON.parse(jsonString);
        if (isFieldType(prop?.type, 'array') && !Array.isArray(parsed)) {
          errors.value[key] = 'Должен быть массив';
          isValid = false;
          return;
        }
        if (isFieldType(prop?.type, 'object') && (Array.isArray(parsed) || typeof parsed !== 'object')) {
          errors.value[key] = 'Должен быть объект';
          isValid = false;
          return;
        }
